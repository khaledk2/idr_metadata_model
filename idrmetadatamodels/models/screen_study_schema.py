# Auto generated from screen_study_schema.yaml by pythongen.py version: 0.0.1
# Generation date: 2024-12-22T12:46:23
# Schema: screen_study_idr_schema
#
# id: idr.screen_study.schema
...
@dataclass
class ScreenStudy(Study):
    """
    Screen study
    """
    _inherited_slots: ClassVar[List[str]] = []

    class_class_uri: ClassVar[URIRef] = LINKML["ScreenStudy"]
    class_class_curie: ClassVar[str] = "linkml:ScreenStudy"
    class_name: ClassVar[str] = "Screen_study"
    class_model_uri: ClassVar[URIRef] = LINKML.ScreenStudy

    id: Union[int, ScreenStudyId] = None
    imaging_method: str = None
    study_type: str = None
    license: Union[dict, License] = None
    data_publisher: str = None
    ...
    screen_technology_type: str = None
    screen_type: str = None

    def __post_init__(self, *_: List[str], **kwargs: Dict[str, Any]):
        if self._is_empty(self.id):
            self.MissingRequiredField("id")
        if not isinstance(self.id, ScreenStudyId):
            self.id = ScreenStudyId(self.id)

        if self._is_empty(self.screen_description):
            self.MissingRequiredField("screen_description")
        if not isinstance(self.screen_description, str):
            self.screen_description = str(self.screen_description)

        if self._is_empty(self.screen_technology_type):
            self.MissingRequiredField("screen_technology_type")
        if not isinstance(self.screen_technology_type, str):
            self.screen_technology_type = str(self.screen_technology_type)

        if self._is_empty(self.screen_type):
            self.MissingRequiredField("screen_type")
        if not isinstance(self.screen_type, str):
            self.screen_type = str(self.screen_type)
...
slots.screenStudy__screen_description = Slot(uri=LINKML.screen_description, name="screenStudy__screen_description", curie=LINKML.curie('screen_description'),
                   model_uri=LINKML.screenStudy__screen_description, domain=None, range=str)

slots.screenStudy__screen_technology_type = Slot(uri=LINKML.screen_technology_type, name="screenStudy__screen_technology_type", curie=LINKML.curie('screen_technology_type'),
                   model_uri=LINKML.screenStudy__screen_technology_type, domain=None, range=str)

slots.screenStudy__screen_type = Slot(uri=LINKML.screen_type, name="screenStudy__screen_type", curie=LINKML.curie('screen_type'),
                   model_uri=LINKML.screenStudy__screen_type, domain=None, range=str)
